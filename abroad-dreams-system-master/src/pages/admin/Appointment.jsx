...
export default function Appointment() {
  ...
  const fetchAppointmentById = async (id) => {
    try {
      const response = await axios.get(
        `http://localhost:8080/appointments/getById/${id}`
      );
  ...
  const handleRemoveAppointment = (appointmentId) => {
    ...
    axios
      ...
      .then((response) => {
        console.log(
          `Appointment with ID ${appointmentId} removed successfully`
        );
        fetchAppointments();
      })
      .catch((error) => {
        console.error(
          `Error removing Appointment with ID ${appointmentId}:`,
          error
        );
      });
  };
  ...
  const handleUpdateAppointment = () => {
    axios
      .put(
        `http://localhost:8080/appointments/update/${editAppointmentId}`,
        editAppointmentData
      )
      .then((response) => {
      ...
      <Container fluid className="flex-grow-1">
        ...
        <div className="appointment-component">
          <div className="d-flex align-items-center mb-3">
            <button
              className="btn btn-dark mr-2 m-1"
              onClick={() => {
                handleShow("addAppointment");
              }}
            >
              Add New Appointment +
            </button>
          ...
          {Array.isArray(appointments) &&
            appointments.map((appointment) => (
              <div className="item" key={appointment.appointmentId}>
                {<strong>ID: {appointment.appointmentId}</strong>}{" "}
                {appointment.purpose}
                {" -- "}
                {appointment.status}
                <div>
                  ...
                  <button
                    ...
                    onClick={() =>
                      handleRemoveAppointment(appointment.appointmentId)
                    }
                  >
                    Remove
                  </button>
              ...
              <>
                ...
                <Modal.Body>
                  <Form>
                    ...
                    <Form.Group
                      className="mb-3"
                      controlId="formEditAppointmentStudentId"
                    >
                    ...
                    <Form.Group
                      className="mb-3"
                      controlId="formEditAppointmentDate"
                    >
                      <Form.Label>Date</Form.Label>
                      <DatePicker
                        selected={
                          editAppointmentData.date
                            ? new Date(editAppointmentData.date)
                            : null
                        }
                        onChange={(date) =>
                          setEditAppointmentData({
                    ...
                    <Form.Group
                      className="mb-3"
                      controlId="formEditAppointmentPurpose"
                    >
                    ...
                    <Form.Group
                      className="mb-3"
                      controlId="formEditAppointmentStatus"
                    >
              ...
              <>
                ...
                <Modal.Body>
                  <Form>
                    ...
                    <Form.Group
                      className="mb-3"
                      controlId="formAppointmentStudentId"
                    >
                      <Form.Label>DashboardStudent ID</Form.Label>
                      <Form.Control
                        type="text"
                    ...
                    <Form.Group
                      className="mb-3"
                      controlId="formAppointmentPurpose"
                    >
                      <Form.Label>Purpose</Form.Label>
                      <Form.Control
                        type="text"
                    ...
                    <Form.Group
                      className="mb-3"
                      controlId="formAppointmentStatus"
                    >
                      <Form.Label>Status</Form.Label>
                      <Form.Control
                        as="select"
